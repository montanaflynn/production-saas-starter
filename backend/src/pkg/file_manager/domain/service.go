...
import (
	...
	"github.com/moasq/backend/pkg/file_manager"
)
...
func (s *fileService) UploadFile(ctx context.Context, req *FileUploadRequest, content io.Reader) (*FileAsset, error) {
	...
	if !file_manager.IsAllowedFileType(sanitizedFilename) {
		return nil, fmt.Errorf("file type not allowed: %s", sanitizedFilename)
	}

	// Get file category
	category := file_manager.GetFileCategory(sanitizedFilename)

	// SECURITY: Check file size limits
	maxSize := file_manager.GetMaxFileSize(category)
	if req.Size > maxSize {
...
func generateFilePath(category file_manager.FileCategory, context file_manager.FileContext, filename string) string {
